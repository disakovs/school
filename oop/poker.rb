...
class PokerHand
  ...
  def full_house?
    !four_of_a_kind? && @ranks.uniq.size == 2
  end
  ...
  def straight?
    sorted_values = []
    @hand.sort.each { |card| sorted_values << card.value }
    test_array = [sorted_values.first]
    4.times do 
      test_array << test_array.last + 1
    end
    test_array == sorted_values
  end
